...
def extract_and_save_all_chars_parallel(
    ...
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        futures = {
            executor.submit(extract_and_save, trie, (c,)): c
                for c in ALLOWED
        }
...
def get_sorted_values_from_trie(trie : datrie.Trie) -> np.ndarray:
    ...
    values = [
        abs(v) for sublist in trie.values() for v in sublist
    ]
    ...
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        work = [
            (trie, f'{directory}/wiki-offsets-{i:03}.keys')
                for (i, trie) in enumerate(tries) if trie is not None
        ]

        futures = {
            executor.submit(
                save_trie_values_as_raw_64bit_keys,
                trie,
                path
            ): path
...
def load_freq_tries_parallel(directory : str, max_threads : int = 0) -> List[datrie.Trie]:
    if max_threads < 1:
        max_threads = os.cpu_count()
